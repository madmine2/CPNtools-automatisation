...
# Checking on argument when calling the file
if len(sys.argv) > 1:
    EXCEL_WITH_REVEIL = sys.argv[1]
    if EXCEL_WITH_REVEIL != "True" and EXCEL_WITH_REVEIL != "False":
        print("wrong argument passed while trying to run the file, give True (excel file with waking up times) or False/ no argument (no wake up times) as argument")
        exit()
    EXCEL_WITH_REVEIL = bool(EXCEL_WITH_REVEIL)
else : 
    EXCEL_WITH_REVEIL = False
print(EXCEL_WITH_REVEIL)

# Functions to write in .cpn file
def filtrer_et_modifier_cpn_reveil():
    ...
    with open('output/chwapi_avec_reveil_modifie.cpn', 'w') as fichier_cpn_modifie:
...
def filtrer_et_modifier_cpn():
    ...
    valeurs_colonne_1 = df_filtre['1`'].tolist()
    valeurs_colonne_1_concatenees = "".join(map(str, valeurs_colonne_1))
    valeurs_colonne_1_concatenees = valeurs_colonne_1_concatenees[:-4]
    valeurs_colonne_1_modifiees = "\n" + valeurs_colonne_1_concatenees.replace("`(", "`\n(")
    ...
    # Écrire le contenu modifié dans un nouveau fichier .cpn
    with open('output/chwapi_modifie.cpn', 'w') as fichier_cpn_modifie:
...
if EXCEL_WITH_REVEIL :  
    ...
    #lecture du fichier .cpn existant
    with open('templates/chwapi.cpn', 'r') as fichier:
        cpn_file = fichier.read()
    # lecture du fichier excel
...
if EXCEL_WITH_REVEIL : 
    bouton_appliquer = ttk.Button(root, text="Appliquer", command=filtrer_et_modifier_cpn_reveil)
else : 
